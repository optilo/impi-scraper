...
import { IMPIError, } from './types.js';
/**
 ...
 */
export async function searchTrademarks(query, options = {}) {
 ...
 */
export function parseIMPISearchUrl(url) {
    try {
 ...
 * @returns Search results with metadata
 */
export async function searchByUrl(url, options = {}) {
    const client = new IMPIApiClient(options);
    try {
 ...
 */
export async function countTrademarks(query, options = {}) {
    const client = new IMPIApiClient(options);
    try {
...
function parseCountResponse(data, url) {
...
export class IMPIApiClient {
     ...
     */
    async resolveAutoProxy() {
        if (this.proxyResolved)
            return;
        this.proxyResolved = true;
        if (this.rawProxyOption !== 'auto')
            return;
    ...
    async initSession() {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt === maxRetries) {
                    throw new Error(`Failed to connect to IMPI after ${maxRetries} attempts: ${lastError.message}\n` +
                        `  This may indicate:\n` +
        ...
        if (!xsrfCookie || !jsessionCookie || !sessionCookie) {
            const missing = [];
            if (!xsrfCookie)
                missing.push('XSRF-TOKEN');
            if (!jsessionCookie)
                missing.push('JSESSIONID');
            if (!sessionCookie)
            ...
            throw createError('SESSION_EXPIRED', `Failed to obtain session tokens. Missing: ${missing.join(', ')}. URL: ${currentUrl}`, { url: IMPI_CONFIG.searchUrl });
        }
    ...
    async getCount(query) {
        ...
        if (!response.ok) {
            throw createError(response.status === 429 ? 'RATE_LIMITED' : 'SERVER_ERROR', `Search count API error: HTTP ${response.status}`, { httpStatus: response.status, url: IMPI_CONFIG.searchCountApiUrl });
        }
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                if (attempt === maxRetries) {
                    ...
                    throw new Error(`Failed to perform quick search after ${maxRetries} attempts: ${lastError.message}\n` +
                        `  This may indicate:\n` +
                        `  - IMPI is blocking the proxy IP\n` +
                        `  - Network connectivity issues\n` +
                        `  - Proxy authentication problems\n` +
     ...
     */
    async getSearchResults(searchId, pageNumber = 0, pageSize = 100) {
        ...
        if (!response.ok) {
            throw createError(response.status === 429 ? 'RATE_LIMITED' : 'SERVER_ERROR', `Search results API error: HTTP ${response.status}`, { httpStatus: response.status, url: IMPI_CONFIG.searchApiUrl });
        }
        const data = await response.json();
        return data;
    }
     ...
     */
    async getTrademarkDetails(impiId, searchId) {
        ...
        if (!response.ok) {
            throw createError(response.status === 429 ? 'RATE_LIMITED' :
                response.status === 404 ? 'NOT_FOUND' : 'SERVER_ERROR', `Details API error: HTTP ${response.status}`, { httpStatus: response.status, url });
        }
        const data = await response.json();
        return data;
    }
        ...
        if (this.options.detailLevel === 'full') {
            for (const trademark of toProcess) {
                try {
                    ...
                    log.warning(`Failed to get details for ${trademark.id}: ${err.message}`);
            ...
            for (let i = 0; i < toProcess.length; i += detailsConcurrency) {
                ...
                const batchPromises = batch.map(async (basicResult, batchIndex) => {
                    ...
                    try {
                        ...
                        log.warning(`Failed to get details for ${basicResult.impiId}: ${err.message}`);
                ...
                for (const { result } of batchResults) {
                    ...
                    if (result.history) {
                        for (const hist of result.history) {
                            if (hist.pdfUrl)
                                pdfUrls.push(hist.pdfUrl);
                            if (hist.oficios) {
                                for (const oficio of hist.oficios) {
                                    if (oficio.pdfUrl)
                                        pdfUrls.push(oficio.pdfUrl);
                                }
                            }
                        }
                    }
     ...
     */
    processRawResults(rawResults, queryLabel, searchId) {
        const results = [];
        if (this.options.detailLevel === 'full') {
            ...
            for (const trademark of rawResults) {
                try {
                    ...
                    log.warning(`Failed to process ${trademark.id}: ${err.message}`);
                    results.push(this.extractBasicData(trademark, queryLabel, searchId));
                }
            }
        }
     ...
     */
    buildSearchResults(queryLabel, searchId, url, totalResults, results, startTime) {
        const duration = Date.now() - startTime;
        return {
            metadata: {
                query: queryLabel,
     ...
     */
    extractBasicData(trademark, query, searchId) {
     ...
     */
    extractTrademarkData(trademark, details, query, searchId) {
 ...
 * ```
 */
export async function generateSessionTokens(options = {}) {
    const { headless = true, proxy, humanBehavior = true, } = options;
    ...
    try {
        ...
        if (!xsrfCookie || !jsessionCookie || !sessionCookie) {
            ...
            if (!sessionCookie)
                missing.push('SESSIONTOKEN');
            throw createError('SESSION_EXPIRED', `Failed to obtain session tokens. Missing: ${missing.join(', ')}`, { url: IMPI_CONFIG.searchUrl });
        }
 ...
 * ```
 */
export async function generateSearchId(query, options = {}) {
    const { headless = true, proxy, humanBehavior = true, tokens: providedTokens, } = options;
 ...
 * ```
 */
export async function generateSearch(query, options = {}) {
 ...
 * ```
 */
export async function generateBatchSearch(queries, options = {}) {
    const { headless = true, proxy, humanBehavior = true, delayBetweenSearchesMs = 500, continueOnError = true, } = options;
    const startTime = Date.now();
...
export class IMPIHttpClient {
     ...
     */
    async fetchSearchResults(searchId, pageNumber = 0, pageSize = 100) {
        log.debug(`Fetching results page ${pageNumber} for search ${searchId}`);
        const response = await this.apiFetch(IMPI_CONFIG.searchApiUrl, {
            method: 'POST',
            headers: {
     ...
     */
    async fetchAllResults(searchId, totalResults, maxResults = 0) {
        const allResults = [];
        const pageSize = 100;
        const totalPages = Math.ceil(totalResults / pageSize);
     ...
     */
    async fetchTrademarkDetails(impiId, searchId) {
        log.debug(`Fetching details for ${impiId}`);
        const params = new URLSearchParams();
        if (searchId) {
            params.set('s', searchId);
     ...
     */
    async fetchAllResultsWithDetails(searchId, totalResults, maxResults = 0) {
     ...
     */
    async processSearch(searchId, totalResults, query, maxResults = 0) {
        const startTime = Date.now();
        if (totalResults === 0 || !searchId) {
            return {
                metadata: {
     ...
     * Extract basic data from raw trademark
     */
    extractBasicData(trademark, searchId) {
        const ownerName = trademark.owners?.[0] || null;
        const classes = trademark.classes?.map(classNum => ({
            classNumber: classNum,
            goodsAndServices: '',
     ...
     * Extract full data from trademark with details
     */
    extractTrademarkData(trademark, details, searchId) {
        const generalInfo = details?.details?.generalInformation;
        const trademarkInfo = details?.details?.trademark;
        const data = {
            searchId,
