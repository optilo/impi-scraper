 ...
 */
export async function detectExternalIp(page) {
 ...
 */
export function resolveProxyConfig(optionsProxy) {
 ...
 */
export function formatProxyForCamoufox(proxy) {
    if (!proxy)
        return undefined;
 ...
 */
export function getProxyErrorMessage(error, proxyServer) {
...
export async function testProxy(proxyConfig) {
    ...
    try {
        ...
        const ipServices = [
            { url: 'http://httpbin.org/ip', parser: (text) => JSON.parse(text).origin?.split(',')[0]?.trim() },
            { url: 'http://ip-api.com/json', parser: (text) => JSON.parse(text).query },
            { url: 'http://api.ipify.org', parser: (text) => text.trim() },
